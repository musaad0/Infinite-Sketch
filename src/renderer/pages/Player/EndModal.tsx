...
export default function EndModal({ showModal, onClose }: Props) {
  ...
  return (
    <Modal isOpen={showModal} onClose={onClose}>
      <Dialog.Title className="text-center text-2xl text-neutral-300">
        Save & Quit ?
      </Dialog.Title>
      <div className="mt-4 flex justify-center gap-4 fill-secondary py-2 text-center text-secondary">
        <Button className="block w-full py-2 px-6" onClick={handleSave}>
          Yes
        </Button>
        <Button className="w-full py-2 px-6" variant="muted" onClick={onClose}>
          Cancel
        </Button>
        <Button className="py-2 px-6" onClick={handleSavedIndex}>
