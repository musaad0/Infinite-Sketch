...
export default function renderFiles(files){
    ...
    tr.innerHTML = `<td><span class="tableText">${files[files.length-1].folderName}</span></td> <td><span class="tableText">${files[files.length-1].filesArr.length}</span></td>`

    tableFiles.appendChild(tr)
    const tmpTotal = files[files.length-1].filesArr.length + parseInt(totNImages.textContent)
